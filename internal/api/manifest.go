...
func manifestCapFor(k manifest.Kind) (c caps.Cap, envScoped bool) {
	...
	case manifest.KindGeneratedSecret:
		// No power of its own: a generated secret exists exactly for the stacks that
		// reference it, and validation guarantees at least one — so the preflight's
		// stacks.edit checks for those stacks ARE its authorization. This mapping
		// exists so the every-kind pin below still answers.
...
type manifestRun struct {
	...
	// genValues caches generated-secret plaintext for the walk: values created in
	// THIS apply, and stored ones unsealed on first reference. Never serialized.
...
func (r *manifestRun) walk(ctx context.Context) error {
	for _, kind := range manifest.Order {
		...
		case manifest.KindGeneratedSecret:
			for _, x := range r.m.GeneratedSecrets {
				if err = r.ensureGeneratedSecret(ctx, x); err != nil {
					return err
				}
			}
		case manifest.KindStack:
...
func (r *manifestRun) ensureGeneratedSecret(ctx context.Context, g manifest.GeneratedSecret) error {
	res := manifestResourceView{Kind: string(manifest.KindGeneratedSecret), Name: g.Name}
	format, length := g.Format, g.Length
	if format == "" {
		format = "alphanumeric"
	}
	if length == 0 {
	...
	existing, err := r.s.store.SecretValueByName(ctx, g.Name)
	switch {
	case err == nil:
		res.ID = existing.ID
		// GENERATED ONCE. Changed parameters are drift, never a regeneration — a
		// silently re-minted password takes every consumer down in undefined order.
		if existing.Format != format || existing.Length != length {
			res.Verdict, res.Detail = verdictDrifted, fmt.Sprintf("exists as %s/%d characters; a generated secret is never regenerated by apply — rotation is a deliberate act", existing.Format, existing.Length)
		} else {
			res.Verdict = verdictInSync
		}
	case errors.Is(err, store.ErrNotFound):
		res.Verdict = verdictCreate
		if r.apply {
			value, err := generateSecretValue(format, length)
			if err != nil {
				return err
			}
			sealed, err := r.s.sealer.Seal(value)
			if err != nil {
				return err
			}
			row := &store.SecretValue{
				Name: g.Name, ValueEnc: sealed, Format: format, Length: length,
				CreatedBy: r.user.ID,
			}
			if err := r.s.store.CreateSecretValue(ctx, row); err != nil {
				return fmt.Errorf("manifest: creating generated secret %q: %w", g.Name, err)
			}
			r.genValues[g.Name] = value
			res.ID = row.ID
			r.auditEnsure(ctx, manifest.KindGeneratedSecret, "", g.Name, verdictCreate)
		}
	default:
		return err
	}
...
// generatedValue resolves a generated secret's plaintext: minted earlier in this walk,
// or unsealed from the store. ok=false means it does not exist YET — legal only during
// a plan, where the walk order guarantees apply would have created it by now.
func (r *manifestRun) generatedValue(ctx context.Context, name string) (string, bool, error) {
	if v, ok := r.genValues[name]; ok {
		return v, true, nil
	}
	row, err := r.s.store.SecretValueByName(ctx, name)
	if errors.Is(err, store.ErrNotFound) {
		return "", false, nil
	}
	if err != nil {
	...
	val, err := r.s.sealer.Open(row.ValueEnc)
	if err != nil {
		return "", false, fmt.Errorf("manifest: could not decrypt generated secret %q (was the master key replaced?)", name)
	}
	r.genValues[name] = val
	return val, true, nil
}

// expandGenerated resolves a composed value's ${generated:…} references.
func (r *manifestRun) expandGenerated(ctx context.Context, s string) (string, bool, error) {
	var resolveErr error
	out, ok := manifest.ExpandGenerated(s, func(name string) (string, bool) {
		v, ok, err := r.generatedValue(ctx, name)
...
// generateSecretValue mints the material with crypto/rand. Alphanumeric draws each
// character independently so the distribution stays uniform.
func generateSecretValue(format string, length int) (string, error) {
	switch format {
	case "hex":
...
func (r *manifestRun) ensureStack(ctx context.Context, st manifest.Stack) error {
	...
	secChanged, err := r.ensureStackSecrets(ctx, existing, st)
	if err != nil {
		return err
	}
...
func (r *manifestRun) ensureStackEnv(ctx context.Context, row *store.Stack, st manifest.Stack) (bool, error) {
	...
	for _, v := range st.Env {
		...
		case v.FromGenerated != "":
			// Generated-backed slots are OWNED: apply converges them to the stored
			// value, which is how one value reaches every consumer — and how a
			// future rotation propagates.
			val, ok, err := r.generatedValue(ctx, v.FromGenerated)
			if err != nil {
				return changed, err
			}
			if ok {
				...
				changed = true // plan: fills once this document's creations exist
			}
		case v.Value != "":
			// Composed (validation guarantees it references a generated secret).
			val, ok, err := r.expandGenerated(ctx, v.Value)
			if err != nil {
				return changed, err
			}
			if ok {
...
// ensureStackSecrets converges the stack's OWNED secret files — the from_generated
// and composed forms. Slot-form files are untouched (reportStackSlots handles them),
// and rows the manifest does not declare are preserved: this is a merge, never a
// replace, exactly like ensureStackEnv.
func (r *manifestRun) ensureStackSecrets(ctx context.Context, row *store.Stack, st manifest.Stack) (bool, error) {
	if row == nil {
		return false, nil
	}
	type ownedFile struct{ name, content string }
	var owned []ownedFile
	changed := false
	for _, sf := range st.SecretFiles {
		if sf.FromGenerated == "" && sf.Value == "" {
			continue // a slot, a human's to fill
		}
		var content string
		var known bool
		var err error
		if sf.FromGenerated != "" {
			content, known, err = r.generatedValue(ctx, sf.FromGenerated)
		} else {
			content, known, err = r.expandGenerated(ctx, sf.Value)
		}
		if err != nil {
			return changed, err
		}
		...
		owned = append(owned, ownedFile{sf.Name, content})
	}
	if len(owned) == 0 {
		return changed, nil
	}

	stored, err := r.s.store.StackSecrets(ctx, row.ID)
	if err != nil {
		return changed, err
	}
	...
	for _, o := range owned {
		if i, ok := byName[o.name]; ok {
			cur, err := r.s.sealer.Open(stored[i].ContentEnc)
			if err != nil {
				return changed, fmt.Errorf("manifest: could not decrypt secret %s on %q (was the master key replaced?)", o.name, st.Name)
			}
			if cur == o.content {
				continue
			}
			sealed, err := r.s.sealer.Seal(o.content)
			if err != nil {
				return changed, err
			}
			stored[i].ContentEnc = sealed
			changed = true
		} else {
			sealed, err := r.s.sealer.Seal(o.content)
			if err != nil {
			...
			stored = append(stored, store.StackSecret{Name: o.name, ContentEnc: sealed})
			changed = true
		}
	}
	if changed && r.apply {
		if err := r.s.store.SetStackSecrets(ctx, row.ID, stored); err != nil {
			return changed, fmt.Errorf("manifest: storing secrets for %q: %w", st.Name, err)
		}
	}
	return changed, nil
}
...
// they simply stay unfilled until someone PUTs content. Generated-backed slots are
// not slots at all: apply fills them, so they never appear here.
func (r *manifestRun) reportStackSlots(ctx context.Context, st manifest.Stack, cluster string, row *store.Stack) error {
	...
	for _, v := range st.Env {
		if !v.Secret || v.FromGenerated != "" || v.Value != "" {
			continue
		}
		e, ok := envByKey[v.Key]
		if !ok {
	...
	for _, sf := range st.SecretFiles {
		...
		if !haveSecret[sf.Name] {
			unfilled("stack_secret_file", sf.Name)
		}
	}
	return nil
}
