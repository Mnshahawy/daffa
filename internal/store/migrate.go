...
`},
	...
	// Generated secrets: values Daffa mints at first manifest apply and injects into
	// every slot that references them. One row per name, sealed under the master key —
	// the same at-rest posture as every other secret. format/length are recorded so a
	// later apply that declares different parameters reads as drift instead of being
	// silently ignored; the value itself is NEVER regenerated by apply.
	{name: "0019_secret_values", sql: `
CREATE TABLE secret_values (
    id         TEXT PRIMARY KEY,
    name       TEXT NOT NULL UNIQUE,         -- global, like the credential stores
    value_enc  TEXT NOT NULL,                -- sealed; write-only through the API
    format     TEXT NOT NULL,                -- alphanumeric | hex | base64
