...
export interface Toast {
  id: number
  /** Reuses the shared status vocabulary so a toast's colour matches every pill. */
 ...
 *
 * Duck-typed rather than importing the app's ApiError class: every console's generated api
 * transport throws an Error subclass carrying a numeric `status` and the server's operator-
 * facing `message` — that shape, not the class identity, is the contract. Anything else is
 * unexpected and gets the caller's fallback rather than a raw stack nobody can act on.
 */
